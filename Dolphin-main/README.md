...
1. **`KeyError: 'DolphinSpeech2Text' object has no attribute 'partial_ar'`**
   - **Cause**: This occurs due to a version mismatch with the `espnet` dependency.
   - **Solution**: Ensure you have the correct version of `espnet` installed (202402). Run:
     ```shell
...
2. **`ModuleNotFoundError: No module named 'flash_attn'`**
   - **Cause**: Missing optional dependency for faster inference.
   - **Solution**: This is a warning, not an error. The model will fall back to ESPnet's default implementation. To install:
     ```shell
     pip install flash-attn
     ```

3. **`sentencepiece` compilation errors**
   - **Cause**: Missing build tools or incompatible Python version.
   - **Solution**: Install build essentials and ensure Python 3.8+ is used:
     ```shell
     # Ubuntu/Debian
